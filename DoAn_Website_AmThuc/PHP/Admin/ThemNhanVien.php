<?php
    ...
    if (isset($_POST["btn_add"])) {
        ...
        if ($sta->rowCount() > 0) {
            ...
            $sql = "INSERT INTO nhanvien(TenNV, SDT, Email, NgaySinh, GioiTinh, DiaChi, MatKhau, Hinh) VALUES ('$ten', '$sdt', '$email', '$ns', '$gt', '$dc', $mk, '$hinh')";
           
            $sta = $pdo->prepare($sql);
            $kq = $sta->execute();
            if ($kq) {
                $tbaddnv = "Thêm nhân viên thành công";
                if ($hinh != "") {
                    move_uploaded_file($_FILES["hinh"]["tmp_name"], "../../assets/user/images/nhanvien/$hinh");
                }
            } else {
                $tbaddnv = "Thêm không thành công! Vui lòng kiểm tra lại dữ liệu";
            }
        }
    }
?>